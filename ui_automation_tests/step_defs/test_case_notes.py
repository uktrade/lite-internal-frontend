...
def n_characters_remaining(driver, text):
    ...
    if text == "disabled":
        assert (
            "disabled" in application_page.get_class_name_of_post_note()
        ), "post note button is not disabled"
    else:
        assert (
            "disabled" not in application_page.get_class_name_of_post_note()
        ), "post note button is disabled"
...
def post_note_is_disabled(driver):
    application_page = ApplicationPage(driver)
    assert (
        application_page.get_disabled_attribute_of_post_note() == "true"
    ), "post note is not disabled but should be"
...
def entered_text_no_longer_in_case_field(driver, context):
    application_page = ApplicationPage(driver)
    assert (
        context.text not in application_page.get_text_of_case_note_field()
    ), "cancel button hasnt cleared text"
